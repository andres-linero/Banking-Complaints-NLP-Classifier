...
covers training, evaluation, and reproducibility, followed by the demo and serving interfaces.
...
See the [routing configuration](#routing-configuration) for how serving
...
Follow the [quick start](#quick-start) first to install dependencies and produce the saved
model and frozen test set. The app loads that model; it does not train a new one.
